<?php
	...
	{
		$sql = "select bw.card_id,b.isbn,b.title from book as b NATURAL JOIN book_loans as bw where date_in is null and isbn in 
		(select isbn from book_loans natural join borrower where isbn like '%$card_id%' or bname like '%$card_id%' or
		card_id=CAST('$card_id' AS UNSIGNED)) group by isbn";
		$result = $con->query($sql);
